...
const particleWGSL = await fetch('particle.wgsl').then(response => response.text());
const probabilityMapWGSL = await fetch('probabilityMap.wgsl').then(response => response.text());
...
let numMipLevels = 1;
{
  const response = await fetch('./scene/Box.png');
