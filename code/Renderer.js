...
const lightBindGroupLayout = {
  entries: [
    ...
    { binding: 1, visibility: GPUShaderStage.FRAGMENT, texture: { sampleType: 'depth' } }, // Change to depth type
...
export class Renderer extends BaseRenderer {
  ...
  async initialize() {
    ...
    this.shadowMapTexture = this.device.createTexture({
      size: [1024, 1024], // Shadow map resolution
      format: "depth24plus",
  ...
  renderShadowMap(scene, light) {
    ...
    passEncoder.setBindGroup(0, this.lightBindGroup); // Contains light view/projection matrices
    this.renderScene(scene, passEncoder, light.getLightMatrix());
  ...
  render(scene, camera, light) {
    ...
    // const lightNode = scene.find((node) => node.getComponentOfType(Light));
    // const lightComponent = light.getComponentOfType(Light);
    const lightNode = scene.find((node) => node.getComponentOfType(Light));

    if (!lightNode) {
      console.error("Light node not found in the scene");
      return; // Exit early if no light node is found
    }

    const lightComponent = lightNode.getComponentOfType(Light);

    if (!lightComponent) {
      console.error("Light component not found in the light node");
      return; // Exit early if no light component is found
    }
    ...
    this.renderShadowMap(scene, lightNode); // Render shadows
