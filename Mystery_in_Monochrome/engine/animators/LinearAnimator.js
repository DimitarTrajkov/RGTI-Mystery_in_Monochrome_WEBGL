...
export class LinearAnimator {
    constructor(nodes, {
        ...
        this.playing = false;
        this.animationStartTime = null;  // Track animation start time
        this.nextAnimations = [];        // Array of follow-up animations
        this.startPositions = new Map(); // Store start positions for each node
        this.endPositions = new Map();   // Store end positions for each node
    }
    ...
    // Add a follow-up animation to the queue
    addNextAnimation(animation) {
    ...
    update(t, dt) {
        ...
        if (!this.loop && linearInterpolation >= 1) {
            ...
            // // Start the next animation in the array if available
            // if (this.nextAnimations.length > 0) {
            //     const nextAnimation = this.nextAnimations.shift(); // Get the next animation
            //     nextAnimation.play(); // Start the next animation
            // }
            for (const animation of this.nextAnimations){
                animation.play();
            }
